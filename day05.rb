...
def read_instructions
  File.open('input/day05.txt').readlines.map { |string| string.to_i }
end


def jump_through_trampoline_maze(twisty = false)
  instructions = read_instructions
  i = 0
  steps = 0
  while i >= 0 and i < instructions.length
    j = i + instructions[i]
    if twisty and instructions[i] >= 3
      instructions[i] -= 1
    else
      instructions[i] += 1
    end
