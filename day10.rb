...
def read_input
  File.open('input/day10.txt').readline.strip
end
...
class CircularList
  ...
  def []=(i, value)
    @elements[i % @elements.length] = value
  end

  def reverse(i, length)
    j = i + length - 1
    (length / 2).times do |k|
...
def knot_hash(input)
  ...
  spare_hash = list
  dense_hash = (0..15).map { |k| spare_hash.hash_block(k * 16) }
  knot_hash = ''
  dense_hash.each { |h| knot_hash += sprintf('%02x', h) }
  knot_hash
end
