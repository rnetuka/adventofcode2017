...
# 5 million pairs. (Using the values from the example above, after five million pairs, the judge would eventually find a
...
def count(n, modulo_a: 1, modulo_b: 1)
  factor_a = 16_807
  factor_b = 48_271
  mod = 2_147_483_647
  a = nil
  b = nil
  init_a = $start_a * factor_a
  init_b = $start_b * factor_b
  score = 0
  mask = 0b1111_1111_1111_1111
  n.times do
    while true
      a = a.nil? ? init_a : (a * factor_a) % mod
      break if a % modulo_a == 0
    end
    while true
      b = b.nil? ? init_b : (b * factor_b) % mod
      break if b % modulo_b == 0
    end
    score += 1 if (a & mask) == (b & mask)
  end
  score
end

puts 'Day 15'
puts " - What is the judge's final count? #{count(40_000_000)}"
puts " - What is the judge's final count? #{count(5_000_000, modulo_a: 4, modulo_b: 8)}"