...
class Memory
  ...
  def next_step
    ...
    infinite_loop = @history.include?(@banks)
    @history << @banks.clone
    if infinite_loop
      raise InfiniteLoopError.new
    end
  end
...
def read_input
  line = File.open('input/day06.txt').readline
  banks = []
  scanner = StringScanner.new(line)
  until scanner.eos?
    banks << scanner.scan(/[0-9]+/).to_i
    scanner.scan(/\s+/)
  end
  banks
end
...
def solution_1
  memory = Memory.new
  memory.run_reallocation
end

def solution_2
  memory = Memory.new
  memory.run_reallocation
  i = memory.history.find_index(memory.history[-1])
  j = memory.history.length - 1
  j - i
end
